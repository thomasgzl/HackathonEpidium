...
!function(exports){
  ...
  raw.model = function() {
    ...
    model.instruction = function() {
      return dimensions.values()
        .filter(function(d) { return d.required() > d.value.length; })
        .map(function(d) { var v = d.required() - d.value.length > 1 ? 'dimensions' : 'dimension'; return '<b>'+d.title() + "</b> requiert au moins " + (d.required() - d.value.length) + " autre " + v;  })
        .join(". ")
    }
  ...
  raw.models.set('tree', function(){
    ...
    var hierarchy = tree.dimension('hierarchy')
       .title('Hiérarchie')
       .description("Description de la hiérarchie qui illustre l'utilité de cette dimension.")
       .required(1)
       .multiple(true);
